...
async def analyze_market(req: MarketAnalysisRequest):
    try:
        ...
        if req.source == "url":
            ...
            item_id = ml_api.extract_item_id_from_url(req.product_url)
            if not item_id:
                raise HTTPException(status_code=400, detail="Não foi possível extrair o ID do produto da URL informada.")

            target_item = await ml_api.get_item_details(item_id)
            search_query = target_item.get("title", req.query)
            ...
            # Posição do anúncio alvo no mercado
            ...
            query_for_insights = target_item.get("title", req.query)
